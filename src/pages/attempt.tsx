              ...
              <div className="flex flex-col">
                {!!questions?.length && (
                  <Question
                    ...
                    submitText={
                      nextQuestionId ? "Save & Next" : "Save & Submit"
                    }
                ...
                {isLoadingQuestions ? (
                  ...
                  <div className="flex flex-col divide-y *:py-4 first:*:pt-0 last:*:pb-0">
                    <div className="text-2xl font-bold">
                      Questions ({questions.filter((q) => !!q.answer).length}/
                      {questions.length})
                    </div>
                    <div className="grid grid-cols-6 gap-4 justify-items-center">
                      {questions.map((question, index) => (
                        <QuestionPill
                          ...
                          onClick={() => {
                            setSearchParams({ question_id: question.id });
                          }}
                        />
                      ))}
                    </div>
